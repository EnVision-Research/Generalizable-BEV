...
class UDA_DA_Lyft(Dataset):
    def __init__(self,
        ...
        self.data_infos_target = self.mark_Domain(self.data_infos_target, Mask_Label=True)
        self.data_infos_source = self.mark_Domain(self.data_infos_source, Mask_Label=False)

        if 'lyft' in self.ann_file_source:
            self.source_cbgs_index = mmcv.load('./data/lyft/lyft_train_cbgs.pkl')
        elif 'nus' in self.ann_file_source:
            self.source_cbgs_index = mmcv.load(
                './data/nuscenes/nus_train_cbgs.pkl')
        elif 'DeepAccident' in self.ann_file_source:
            self.source_cbgs_index = mmcv.load(
                './data/DeepAccident_data/da_train_cbgs.pkl')

        self.data_infos = self.mix_source_target(self.data_infos_source, self.data_infos_target, self.source_cbgs_index)
        self.domain_label = True
        self.small_epoch = 5000

        # print('self.data_infos[0][Target_Domain]', self.data_infos[0]['Target_Domain'])
        # print('self.data_infos[1][Target_Domain]', self.data_infos[1]['Target_Domain'])
        # print('self.data_infos[2][Target_Domain]', self.data_infos[2]['Target_Domain'])
        # print('self.data_infos[3][Target_Domain]', self.data_infos[3]['Target_Domain'])
        # print('self.data_infos[4][Target_Domain]', self.data_infos[4]['Target_Domain'])
        # print('self.data_infos[5][Target_Domain]', self.data_infos[5]['Target_Domain'])
        # print('self.data_infos[6][Target_Domain]', self.data_infos[6]['Target_Domain'])
    ...
    def mark_Domain(self, data_infos_target, Mask_Label=True):
        for info in data_infos_target:
            info['Target_Domain'] = Mask_Label
        return data_infos_target

    def CBGS_idnex(self, data_infos, source_cbgs_index):
        CBGS_list = []
        for i in range(len(source_cbgs_index)):
            CBGS_list.append(data_infos[source_cbgs_index[i]])
        return CBGS_list

    def mix_source_target(self, data_infos_source, data_infos_target, source_cbgs_index):
        # 打乱数据
        data_infos_source = self.CBGS_idnex(data_infos_source, source_cbgs_index)
        random.shuffle(data_infos_target)
        # 判断多少
        if len(data_infos_source) < len(data_infos_target):
            smaller_list = data_infos_source
            larger_list = data_infos_target
        else:
            smaller_list = data_infos_target
            larger_list = data_infos_source
        num_dif = len(larger_list) - len(smaller_list)
        smaller_num = len(smaller_list)
        # balanced the source and target domain
        for index in range(num_dif):
            random_index = random.randint(0, smaller_num-1)
            smaller_list.append(smaller_list[random_index])
        merged_list = []
        for i in range(len(larger_list)):
            merged_list.append(larger_list[i])
    ...
    def get_data_info(self, index):
        ...
        if 'Target_Domain' in info:
            input_dict['Target_Domain'] = info['Target_Domain']
    ...
    def __len__(self):
        ...
        """
        if self.small_epoch > 0:
            return self.small_epoch
        else:
    ...
    def _rand_another(self, idx):
        ...
        """
        if self.small_epoch > 0:
            pool = np.where(self.flag_small == self.flag_small[idx])[0]
        else:
    ...
    def _set_group_flag(self):
        ...
        self.flag_small = np.zeros(len(self.data_infos), dtype=np.uint8)
    ...
    def prepare_train_data(self, index):
        ...
        """
        input_dict = self.get_data_info(index)
        if (self.domain_label == input_dict['Target_Domain']) or (input_dict is None):
            return None
        self.domain_label = input_dict['Target_Domain']
        self.pre_pipeline(input_dict)
