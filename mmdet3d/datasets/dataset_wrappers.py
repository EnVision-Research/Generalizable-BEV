...
class CBGSDataset(object):
    ...
    def __init__(self, dataset):
        ...
        if hasattr(self.dataset, 'flag'):
            self.flag = np.array(
                ...
                dtype=np.uint8)
        # mmcv.dump(self.sample_indices, '/mnt/cfs/algorithm/hao.lu/Code/BEVDepth_DA/data/DeepAccident_data/da_train_cbgs.pkl')
    def _get_sample_indices(self):
        ...
        """
        class_sample_idxs = {cat_id: [] for cat_id in self.cat2id.values()}  # 含义 self.cat2id = {name: i for i, name in enumerate(self.CLASSES)}

        # print('self.cat2id.values()', self.cat2id.values())
        # self.cat2id.values()
        # dict_values([0, 1, 2, 3, 4, 5])
        # print(class_sample_idxs)
        # {0: [], 1: [], 2: [], 3: [], 4: [], 5: []}
