...
class LyftDataset(Custom3DDataset):
    ...
    def evaluate(self,
        ...
        result_files, tmp_dir = self.format_results(results_pre, jsonfile_prefix,
                                                    csv_savepath)
...
@DATASETS.register_module()
class LyftDataset_my(Custom3DDataset):
    r"""Lyft Dataset.
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        lyft_annos = {}
        mapped_class_names = self.CLASSES
        pre_token = mmcv.load('/mnt/cfs/algorithm/hao.lu/lyft2lyft_meta.pkl')
        print('Start to convert detection format...')
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            annos = []
            boxes = output_to_lyft_box(det)
    ...
    def find_index(self, data_infos, sample_token):
        ...
        # token 不包含图片具体样本
        # sample_token
        # dict_keys(['token', 'sample_token', 'timestamp', 'scene_token', 'cam_infos',
        #  'cams', 'lidar2ego_translation', 'lidar2ego_rotation', 'ego2global_translation', 'ego2global_rotation',
        # 'lidar_infos', 'cam_sweeps', 'lidar_sweeps', 'ann_infos', 'ann_infos_v1'])
    ...
    def format_results(self, results, jsonfile_prefix=None, csv_savepath=None):
        ...
        assert len(results) == len(self), (
            ...
            format(len(results), len(self)))
        # print(len(results))
        # mmcv.dump(results, '/mnt/cfs/algorithm/hao.lu/Code/BEVDet/scripts/Eval/format_results1.pkl')
        # 3780
        if jsonfile_prefix is None:
            tmp_dir = tempfile.TemporaryDirectory()
            jsonfile_prefix = osp.join(tmp_dir.name, 'results')
...
@DATASETS.register_module()
class LyftDataset_my1(LyftDataset_my):
...
@DATASETS.register_module()
class LyftDataset_pkl(LyftDataset_my):
